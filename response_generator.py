...
for _ in range(20):
    ...
    for idx, row in tqdm(econ_df.iterrows(), total=len(econ_df)):
        reply, opinion = get_resonse(row['statement'])
        row_data[row['statement_abb']] = opinion
        model_responses.append({str(row['statement_abb']):reply})

    updated_rows.append({
        "model": model,
        **row_data
    })
    responses[model].append(model_responses)
    
cur_data = pd.DataFrame(updated_rows)
old_data = pd.read_csv("pca_data/Statements_Opinion.csv")
pd.concat([cur_data, old_data]).to_csv("pca_data/Statements_Opinion.csv", index=False)

json.dump(responses, open(f"pca_data/{model}.json", "w"), indent=4)
